...
async function handleAdvisorQuery(e: Event) {
    ...
    if (conversationTurns >= MAX_CONVERSATION_TURNS) {
        const supportMessage = `It seems your query is quite specific. For the best assistance, please start a general application and our experts will guide you, or contact them directly at <a href="mailto:vg@vcanresources.com">vg@vcanresources.com</a>.`;
    ...
    const aiResponseSchema = {
        ...
        properties: {
            recommendedProduct: { type: SchemaType.STRING, enum: ['Invoice Financing', 'PO Financing', 'Supply Chain Finance', 'None'] },
    ...
    try {
        ...
        const response = await model.generateContent(`System: You are a trade finance advisor for Vcanship. Recommend 'Invoice Financing', 'PO Financing', or 'Supply Chain Finance'. If unclear, recommend 'None'. Provide a brief reasoning.
