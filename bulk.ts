...
async function handleBulkFormSubmit(e: Event) {
    ...
    // Try Sea Rates API first (real quotes for bulk)
    try {
        ...
        if (quoteContainer && realQuotes.length > 0) {
            ...
            quoteContainer.innerHTML = `
                 <div class="payment-overview">
                    ...
                    <hr>
                    <div class="review-item total"><span>Estimated Charter Cost:</span><strong>${State.currentCurrency.symbol}${bestQuote.totalCost.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</strong></div>
                </div>
            `;
            ...
            setTimeout(async () => {
                ...
                emailContainer.id = 'bulk-email-inquiry-form-container';
                ...
                emailContainer.innerHTML = renderEmailInquiryForm('bulk', realQuotes, { origin, destination: dest, cargoType, quantity });
                attachEmailInquiryListeners('bulk', realQuotes, { origin, destination: dest, cargoType, quantity });
            }, 100);
        }
        goToBulkStep(2);
        return;
